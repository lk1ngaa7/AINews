...
func InitLogger(logpath string, loglevel string) {
	...
	// 构造日志
	BuzzLogger = zap.New(core, caller, development, filed)
	BuzzLogger.Info("DefaultLogger init success")
}
func test() {

	// 强结构形式
	BuzzLogger.Info("test",
		zap.String("string", "string"),
		zap.Int("int", 3),
		zap.Duration("time", time.Second),
	)
	// 必须 key-value 结构形式 性能下降一点
	BuzzLogger.Sugar().Infow("test-",
		"string", "string",
		"int", 1,
		"time", time.Second,
	)
